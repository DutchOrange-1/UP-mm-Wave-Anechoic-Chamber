...
class SettingsManager:
    ...
    def change_move_settings(self) -> None:
        ...
        new_asel = interface_repeater(
            lambda: int(input("Input acceleration: ")))()
        new_decel = interface_repeater(
            lambda: int(input("Input deceleration: ")))()
    ...
    def change_move_settings_calb(self) -> None:
        ...
        new_asel = interface_repeater(
            lambda: float(input("Input acceleration: ")))()
        new_decel = interface_repeater(
            lambda: float(input("Input deceleration: ")))()
    ...
    def change_edges_settings(self) -> None:
        ...
        if (edges_settings.BorderFlags & ximc.BorderFlags.BORDER_IS_ENCODER):
            print(
                "\tBORDER_IS_ENCODER is set: The borders are fixed by predetermined encoder values.")
        else:
            print(
                "\tBORDER_IS_ENCODER is unset: The borders are placed on limit switches.")

        if (edges_settings.BorderFlags & ximc.BorderFlags.BORDER_STOP_LEFT):
            print(
                "\tBORDER_STOP_LEFT is set: The motor stops when it reaches the left border.")
        else:
            print(
                "\tBORDER_STOP_LEFT is unset: The motor continues to move when it reaches the left border.")

        if (edges_settings.BorderFlags & ximc.BorderFlags.BORDER_STOP_RIGHT):
            print(
                "\tBORDER_STOP_RIGHT is set: The motor stops when it reaches the right border.")
        else:
            print(
                "\tBORDER_STOP_RIGHT is unset: The motor continues to move when it reaches the right border.")
        ...
        if key_press == "Y" or key_press == "y":
            try:
                edges_settings.LeftBorder = interface_repeater(
                    lambda: int(input("Enter the left border: ")))()
                edges_settings.RightBorder = interface_repeater(
                    lambda: int(input("Enter the right border: ")))()
            except Exception:
                print("Left border {0}, right border {1}".format(
                    edges_settings.LeftBorder, edges_settings.RightBorder))
